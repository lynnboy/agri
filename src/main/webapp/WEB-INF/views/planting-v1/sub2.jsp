...
function add() {
	var jbox = top.$.jBox;
	jbox.open("iframe:${base}${basepath}add", "添加种植模式面积",
...
function modify(id) {
	var jbox = top.$.jBox;
	jbox.open("iframe:${base}${basepath}modify/" + id, "修改种植模式面积",
			$(top.document).width()-320, 360,{
		id: "模式面积modifydialog",
		buttons:{},
		loaded: function(h) {
...
<c:if test="${mode != 'view'}">
			...
			<button class="btn btn-primary btn-mini"
				onclick="modify('${row['模式代码']}')">修改</button>
			<button class="btn btn-primary btn-mini"
				onclick="remove('${row['模式代码']}')">删除</button>
			</td>
</c:if>
			</tr>
			<tr>
