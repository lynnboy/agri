...
function add() {
	var jbox = top.$.jBox;
	jbox.open("iframe:${base}${basepath}add", "添加作物施肥数据",
...
function modify(id) {
	var jbox = top.$.jBox;
	jbox.open("iframe:${base}${basepath}modify/" + id, "修改作物施肥数据",
			$(top.document).width()-220, $(top.document).height()-200,{
		id: "施肥modifydialog",
		buttons:{},
		loaded: function(h) {
...
<c:if test="${mode != 'view'}">
			...
			<button class="btn btn-primary btn-mini"
				onclick="modify('${row['序号']}')">修改</button>
			<button class="btn btn-primary btn-mini"
				onclick="remove('${row['序号']}')">删除</button>
			</td>
</c:if>
			</tr>
			<tr>
